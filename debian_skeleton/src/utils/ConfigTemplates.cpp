...
namespace VenomTemplates {
    ...
    const std::string KERNEL_HARDENING_PARAMS = 
        const std::string KERNEL_HARDENING_PARAMS = 
        "quiet splash mitigations=full,nosmt " // 'full' a maximális szigorért
        "spectre_v2=on spec_store_bypass_disable=on "
        "gather_data_sampling=force "          // Downfall (GDS) blokkolás
        "reg_file_data_sampling=on "           // RFDS (Intel) védelem
        "zenbleed=on "                         // AMD Zenbleed fix
        "spec_rstack_overflow=safe-ret "       // Inception elleni gát
        "l1tf=full,force mds=full,nosmt tsx=off "
        "page_alloc.shuffle=1 slab_nomerge init_on_alloc=1 init_on_free=1 "
