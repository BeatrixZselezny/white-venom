...
# ---------------------------------------------------------------------------
# Capabilities summary + noninteractive package operations (WV)
# Safe defaults for env -i executions
: "${WV_CAP_KERNEL_LIVE:=0}"
: "${WV_CAP_BOOT_CHAIN:=0}"
: "${WV_CAP_PACKAGE_MGMT:=0}"
: "${WV_CAP_SERVICE_MGMT:=0}"
: "${WV_CAP_FS_IMMUTABLE:=0}"

# Noninteractive package operations (only when enabled)
if [ "${WV_CAP_PACKAGE_MGMT}" = "1" ]; then
  export DEBIAN_FRONTEND=noninteractive
fi

WV_CAPS_SUMMARY="Caps: KERNEL_LIVE=${WV_CAP_KERNEL_LIVE} BOOT_CHAIN=${WV_CAP_BOOT_CHAIN} PACKAGE_MGMT=${WV_CAP_PACKAGE_MGMT} SERVICE_MGMT=${WV_CAP_SERVICE_MGMT} FS_IMMUTABLE=${WV_CAP_FS_IMMUTABLE}"
echo "${WV_CAPS_SUMMARY}" >&2
# ---------------------------------------------------------------------------
...
#  A szkript által létrehozott kritikus fájlok, amiket rollbackelni kell
...
# ---------------------------------------------------------------------------
#  JAVÍTÁS: ROLLBACK FÜGGVÉNY ÉS TRAP ERR
# ---------------------------------------------------------------------------
branch_cleanup() {
